...
class LineFeeder(object):
    ...
    def next(self, direction=1):
        if direction == 1:
            ...
            second_point = self.point_list[
                (self.current_line_index + direction) % self.point_list_len]
        else:
            first_point = self.point_list[
                (self.current_line_index + direction) % self.point_list_len]
    ...
    def next(self, direction=1):
        try:
            if self.at_vertex:
                ...
                self.last_vertex = locations.Point.copy(
                    self.point_feed.location)
...
if __name__ == '__main__':
    lf = PointFeeder([locations.Point(0, 0), locations.Point(
        0, 5), locations.Point(5, 5), locations.Point(5, 0)])
    for i in xrange(3):
        print lf.next(1)
    for i in xrange(10):
        print lf.next(-1)
